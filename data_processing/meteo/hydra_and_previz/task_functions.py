...
project_slug = "hydra"
headers = {"Authorization": f"Bearer {SECRET_SENTRY_API_TOKEN}"}
...
def build_resource_url(dataset_id, resource_id):
    return (
        f"https://www.data.gouv.fr/fr/datasets/{dataset_id}/#/resources/{resource_id}"
    )
...
def get_and_send_errors():
        ...
        for res in r["resources"]:
            if "csv" in res["format"] and res["type"] == "main":
                row = {
                    ...
                    "internal:last_modified": better_parse(
                        res["internal"]["last_modified_internal"]
                    ),
                    "analysis:last-modified-at": better_parse(
                        res["extras"].get("analysis:last-modified-at")
                    ),
                    "analysis:parsing:error": res["extras"].get(
                        "analysis:parsing:error"
                    ),
                    "analysis:parsing:finished_at": better_parse(
                        res["extras"].get("analysis:parsing:finished_at")
                    ),
                    "analysis:parsing:started_at": better_parse(
                        res["extras"].get("analysis:parsing:started_at")
                    ),
                }
                data.append(row)
    ...
    df["parsing_time"] = df.apply(
        lambda x: get_delay_seconds(
            x["analysis:parsing:started_at"], x["analysis:parsing:finished_at"]
        ),
        axis=1,
    )
    df["delay_to_parse"] = df.apply(
        lambda x: get_delay_seconds(
            x["internal:last_modified"], x["analysis:parsing:started_at"]
        ),
        axis=1,
    )
    ...
    sentry_error = should_be_previz.loc[
        ~should_be_previz["analysis:parsing:error"].isna()
    ]
