    ...
    if response.status_code == 200:
        ...
        for td in tds:
            ...
            if i == 2 and found:
                ...
                found = False
                if filename not in max_dates or (
                    filename in max_dates and max_dates[filename] < new_date
                ):
        ...
        for tour in ["1", "2"]:
            for file in ["candidatsT", "resultatsT"]:
                ...
                arr, new_max_dates = parse_http_server(
                    url, max_dates, new_max_dates, arr, file[0].upper() + tour + dep
                )
...
def download_local_files(ti):
    ...
    for cf in miom_files:
        ...
        dest_path = (
            ...
            + "/".join(
                cf["link"].replace(URL_ELECTIONS_HTTP_SERVER, "").split("/")[:-1]
            )
            + "/"
        )
        dest_name = cf["name"]
        ...
        list_files=[
            File(
                source_path=(
                    ...
                    + "/".join(
                        cf["link"]
                        .replace(URL_ELECTIONS_HTTP_SERVER, "")
                        .split("/")[:-1]
                    )
                    + "/"
                ),
                ...
                dest_path=(
                    ...
                    + "/".join(
                        cf["link"]
                        .replace(
                            URL_ELECTIONS_HTTP_SERVER + ID_CURRENT_ELECTION + "/", ""
                        )
                        .split("/")[:-1]
                    )
                    + "/"
...
def create_candidats_files():
    ...
    for typeCandidat in ["candidatsT1", "candidatsT2"]:
        ...
        if df.shape[0] > 0:
            df.to_csv(
                f"{AIRFLOW_DAG_TMP}elections-mirroring/{typeCandidat}.csv", index=False
            )
        else:
...
def manage_data(item, level, typefile, mydict_general, mydict_com, candidat):
    ...
    for tour in item.find_all("Tour"):
        ...
        if typefile == "general":
            ...
            mydict["Abstentions_RapportInscrit"] = (
                tour.find("Abstentions").find("RapportInscrits").text
            )
            mydict["Votants"] = tour.find("Votants").find("Nombre").text
            mydict["Votants_RapportInscrit"] = (
                tour.find("Votants").find("RapportInscrits").text
            )
            mydict["Blancs"] = tour.find("Blancs").find("Nombre").text
            mydict["Blancs_RapportInscrit"] = (
                tour.find("Blancs").find("RapportInscrits").text
            )
            mydict["Blancs_RapportVotant"] = (
                tour.find("Blancs").find("RapportVotants").text
            )
            mydict["Nuls"] = tour.find("Nuls").find("Nombre").text
            mydict["Nuls_RapportInscrit"] = (
                tour.find("Nuls").find("RapportInscrits").text
            )
            mydict["Nuls_RapportVotant"] = tour.find("Nuls").find("RapportVotants").text
            mydict["Exprimes"] = tour.find("Exprimes").find("Nombre").text
            mydict["Exprimes_RapportInscrit"] = (
                tour.find("Exprimes").find("RapportInscrits").text
            )
            mydict["Exprimes_RapportVotant"] = (
                tour.find("Exprimes").find("RapportVotants").text
            )
...
def manage_cases(xml_data, level):
    ...
    base_dict = {"Type": soup.find("Type").text, "Annee": soup.find("Annee").text}
    ...
    for department in soup.find_all("Departement"):
        ...
        for circonscription in department.find_all("Circonscription"):
            ...
            if level == "communes":
                ...
                general_result = manage_data(
                    circonscription, level, "general", circonscription_dict, None, None
                )
                general_results.append(general_result)
...
def create_resultats_files():
    ...
    typeFiles = [
        {"level": "communes", "code": "COM.xml"},
        {"level": "circos", "code": "CIR.xml"},
    ]
    for tf in typeFiles:
        print("-----", tf["level"])
        for typeResultat in ["resultatsT1", "resultatsT2"]:
