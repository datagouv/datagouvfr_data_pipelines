...
time wc -l /srv/sirene/data-sirene/data/dep_*.csv | sort -n -r | grep dep | sed 's/^.*_\(.*\).csv/\1/' | \
  parallel -j 36 -t /srv/sirene/venv/bin/python geocode.py /srv/sirene/data-sirene/data/dep_{}.csv /srv/sirene/data-sirene/data/geo_siret_{}.csv.gz /srv/sirene/data-sirene/cache_geo/cache_addok_sirene_{}.csv.db \> /srv/sirene/data-sirene/data/geo_siret_{}.log
echo "Geocode OK!"
