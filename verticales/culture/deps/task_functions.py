...
def get_perimeter(**context):
    """Identifiants des jeux de données rattachés au topic. Le contenu d'un topic
    est exposé par la sous-ressource paginée `/elements/` ; un élément peut
    pointer vers une URL externe ou être marqué « donnée non trouvée », d'où le
    `element` potentiellement nul."""
    ids = []
    url = f"{local_client.base_url}/api/2/topics/{topic_id}/elements/?page_size=100"
    while url:
        r = requests.get(url, headers=HEADERS)
        r.raise_for_status()
        payload = r.json()
        for element in payload["data"]:
            target = element.get("element") or {}
            if target.get("class") == "Dataset":
    ...
    context["ti"].xcom_push(key="datasets", value=sorted(set(ids)))
