    ...
    if isinstance(jsonschema, dict):
        for k in jsonschema:
            if k == "dependencies":
                ...
                dependencies.update(
                    get_dependencies(jsonschema[k], parent=parent + "." + k)
                )
    return dependencies
    ...
    def _process(jsonschema, indent=indent, parent=parent, indent_char=indent_char):
        ...
        for prop in jsonschema["properties"].keys():
            ...
            if "$ref" in spec:
                ...
                md += (
                    indent * indent_char
                    + f"#### {title + ' - ' if title else ''}Propriété {prop}\n"
                )
                if spec.get("description"):
                    md += indent * indent_char + "> *Description : {}*<br>\n".format(
                        spec["description"].strip().replace("\n", " ")
                    )
            ...
            if "properties" in spec:
                if not processed:
                    ...
                    md += (
                        indent * indent_char + f"- Type : {translate_types['object']}\n"
                    )
                ...
                md += _process(
                    {
                        ...
                        "definitions": jsonschema.get("definitions", {}),
                    },
                    indent=indent + 1,
                    parent=parent + "." + prop,
                )
            ...
            if "items" in spec:
                ...
                if "$ref" in items or "title" in items or "description" in items:
                    ...
                    if not processed:
                        ...
                        md += (
                            indent * indent_char
                            + f"- Type : {translate_types.get('array')}\n"
                        )
                    md += accordion("open", indent, indent_char)
                    md += (
            ...
            if "enum" in spec and not processed:
                md += indent * indent_char + f"#### Propriété {prop}\n"
                md += (
                    indent * indent_char + f"- Type : {translate_types.get('array')}\n"
                )
                md += f"{indent * indent_char}- Valeurs autorisées :\n"
