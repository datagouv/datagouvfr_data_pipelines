...
from dag_datagouv_data_pipelines.utils.utils import check_if_first_day_of_month, check_if_monday
...
def compute_top(pattern, period, date, title):
    ...
    for data in r.json():
        if 'url' in data:
            if((data['url'] != 'https://www.data.gouv.fr/fr/datasets/') & (data['url'] != 'https://www.data.gouv.fr/fr/reuses/')):
                r2 = requests.get(data['url'].replace('https://www.data.gouv.fr/fr/','https://www.data.gouv.fr/api/1/'))
                print(data)
                mydict = {}
                ...
                arr.append(mydict)
                textTop = textTop+str('`'+str(data['nb_visits'])+'`').ljust(10)+data['url']+'\n'
    mydict = {}
    mydict['nom'] = title
...
def get_top(ti, **kwargs):
    ...
    textTop, mydict = compute_top(piwik_info['type'], 'range', start.strftime('%Y-%m-%d')+','+end.strftime('%Y-%m-%d'), piwik_info['title'])
...
def getstats(dates, period):
    ...
    for d in dates:
        ...
        if(period == 'month'):
            ...
            matomodate = start_date.strftime('%Y-%m-%d')+','+end_date.strftime('%Y-%m-%d')
        if(period == 'week'):
            matomodate = start_date.strftime('%Y-%m-%d')
        if(period == 'day'):
            matomodate = start_date.strftime('%Y-%m-%d')
        pageviews, uniq_pageviews, downloads = compute_general(matomodate, str(d)[:10], pageviews, uniq_pageviews, downloads)
    return pageviews, uniq_pageviews, downloads
...
def publish_top_mattermost(ti, **kwargs):
    ...
    print(publish_info)
    top_datasets = ti.xcom_pull(key='top_datasets', task_ids='get_top_datasets_'+publish_info['period'])
    top_reuses = ti.xcom_pull(key='top_reuses', task_ids='get_top_reuses_'+publish_info['period'])

    publish_mattermost = MattermostOperator(
            task_id="publish_top_dataset",
            mattermost_endpoint=MATTERMOST_DATAGOUV_REPORTING,
            text=":rolled_up_newspaper: **Top 10 jeux de données** - "+publish_info['periode']+" (visites)\n\n"+top_datasets
    )
    publish_mattermost.execute(dict())

    publish_mattermost = MattermostOperator(
            task_id="publish_top_reuse",
            mattermost_endpoint=MATTERMOST_DATAGOUV_REPORTING,
            text=":artist: **Top 10 réutilisations** - "+publish_info['periode']+" (visites)\n\n"+top_reuses
    )
    publish_mattermost.execute(dict())
...
def send_tops_to_minio(ti, **kwargs):
    publish_info = kwargs.get("templates_dict")
    top_datasets = ti.xcom_pull(key='top_datasets_dict', task_ids='get_top_datasets_'+publish_info['period'])
    top_reuses = ti.xcom_pull(key='top_reuses_dict', task_ids='get_top_reuses_'+publish_info['period'])
...
with DAG(
    ...
    tags=['stats','daily','weekly','monthly','datagouv','piwik'],
    ...
    send_top_day_to_minio = PythonOperator(
        ...
        templates_dict={
            'period': 'day',
            'minio': AIRFLOW_ENV+"/"+MINIO_PATH+'piwik_tops_daily/'+'{{ ds }}'+'/'
        },
    )
    ...
    send_stats_day_to_minio = PythonOperator(
        ...
        templates_dict={
            'period': 'day',
            'minio': AIRFLOW_ENV+"/"+MINIO_PATH+'piwik_stats_daily/'+'{{ ds }}'+'/',
            'date': '{{ ds }}',
    ...
    send_top_week_to_minio = PythonOperator(
        ...
        templates_dict={
            'period': 'week',
            'minio': AIRFLOW_ENV+"/"+MINIO_PATH+'piwik_tops_weekly/'+'{{ ds }}'+'/'
        },
    )
    ...
    send_stats_week_to_minio = PythonOperator(
        ...
        templates_dict={
            'period': 'week',
            'minio': AIRFLOW_ENV+"/"+MINIO_PATH+'piwik_stats_weekly/'+'{{ ds }}'+'/',
            'date': '{{ ds }}',
    ...
    send_top_month_to_minio = PythonOperator(
        ...
        templates_dict={
            'period': 'month',
            'minio': AIRFLOW_ENV+"/"+MINIO_PATH+'piwik_tops_monthly/'+'{{ ds }}'+'/'
        },
    )
    ...
    send_stats_month_to_minio = PythonOperator(
        ...
        templates_dict={
            'period': 'month',
            'minio': AIRFLOW_ENV+"/"+MINIO_PATH+'piwik_stats_monthly/'+'{{ ds }}'+'/',
            'date': '{{ ds }}',
