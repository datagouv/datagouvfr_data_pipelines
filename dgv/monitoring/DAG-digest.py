...
from dag_datagouv_data_pipelines.utils.utils import check_if_monday, check_if_first_day_of_month
...
def get_stats_daily(TODAY):
    with open(AIRFLOW_DAG_TMP+DAG_FOLDER+"digest_daily/"+TODAY+'/output/stats.json') as json_file:
        res = json.load(json_file)
    recap = '- '+str(res['stats']['nb_datasets'])+' datasets créés\n- '+str(res['stats']['nb_orgas'])+' orgas créées\n- '+str(res['stats']['nb_reuses'])+' reuses créées\n- '+str(res['stats']['nb_discussions'])+' discussions créées\n- '+str(res['stats']['nb_users'])+' users créés\n'
    return recap

def publish_mattermost_daily(ti, **kwargs):
    templates_dict = kwargs.get("templates_dict")
    report_url = ti.xcom_pull(key='report_url', task_ids='run_notebook_and_save_to_minio_daily')
...
def get_stats_weekly(TODAY):
    with open(AIRFLOW_DAG_TMP+DAG_FOLDER+'digest_weekly/'+TODAY+'/output/stats.json') as json_file:
        res = json.load(json_file)
    recap = '- '+str(res['stats']['nb_datasets'])+' datasets créés\n- '+str(res['stats']['nb_reuses'])+' reuses créées\n'
    return recap
...
def publish_mattermost_weekly(ti, **kwargs):
    templates_dict = kwargs.get("templates_dict")
    report_url = ti.xcom_pull(key='report_url', task_ids='run_notebook_and_save_to_minio_weekly')
    stats = get_stats_weekly(templates_dict['TODAY'])
    publish_mattermost = MattermostOperator(
            task_id="publish_result",
            mattermost_endpoint=MATTERMOST_DATAGOUV_ACTIVITES,
            text="Weekly Digest : "+report_url+'\n'+stats,
            image_url="https://"+MINIO_URL+"/"+MINIO_BUCKET_DATA_PIPELINE_OPEN+"/"+MINIO_PATH+"digest_weekly/"+templates_dict['TODAY']+"/output/weekly-graph.png"
    )
    publish_mattermost.execute(dict())
...
def get_stats_monthly(TODAY):
    with open(AIRFLOW_DAG_TMP+DAG_FOLDER+'digest_monthly/'+TODAY+'/output/stats.json') as json_file:
        res = json.load(json_file)
    recap = '- '+str(res['stats']['nb_datasets'])+' datasets créés\n- '+str(res['stats']['nb_reuses'])+' reuses créées\n'
    return recap
...
def publish_mattermost_monthly(ti, **kwargs):
    templates_dict = kwargs.get("templates_dict")
    report_url = ti.xcom_pull(key='report_url', task_ids='run_notebook_and_save_to_minio_monthly')
    stats = get_stats_monthly(templates_dict['TODAY'])
    publish_mattermost = MattermostOperator(
            task_id="publish_result",
            mattermost_endpoint=MATTERMOST_DATAGOUV_ACTIVITES,
            text="Monthly Digest : "+report_url+'\n'+stats,
            image_url="https://"+MINIO_URL+"/"+MINIO_BUCKET_DATA_PIPELINE_OPEN+"/"+MINIO_PATH+"digest_monthly/"+templates_dict['TODAY']+"/output/monthly-graph.png"
    )
    publish_mattermost.execute(dict())
...
with DAG(
    ...
    run_nb_daily = PapermillMinioOperator(
        ...
        parameters={
            ...
            "OUTPUT_DATA_FOLDER": AIRFLOW_DAG_TMP+DAG_FOLDER+"digest_daily/"+'{{ ds }}'+"/output/",
            "DATE_AIRFLOW": '{{ ds }}',
            "PERIOD_DIGEST": 'daily'
        }
    )
    ...
    run_nb_weekly = PapermillMinioOperator(
        ...
        parameters={
            "msgs": "Ran from Airflow "+'{{ ds }}'+"!",
            "WORKING_DIR": AIRFLOW_DAG_HOME,
            "OUTPUT_DATA_FOLDER": AIRFLOW_DAG_TMP+DAG_FOLDER+"digest_weekly/"+'{{ ds }}'+"/output/",
            "DATE_AIRFLOW": '{{ ds }}',
    ...
    run_nb_monthly = PapermillMinioOperator(
        ...
        parameters={
            "msgs": "Ran from Airflow "+'{{ ds }}'+"!",
            "WORKING_DIR": AIRFLOW_DAG_HOME,
            "OUTPUT_DATA_FOLDER": AIRFLOW_DAG_TMP+DAG_FOLDER+"digest_monthly/"+'{{ ds }}'+"/output/",
            "DATE_AIRFLOW": '{{ ds }}',
