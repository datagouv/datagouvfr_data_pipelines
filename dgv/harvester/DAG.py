...
def get_pending_harvester_from_api(ti):
    page = 1
    r = requests.get('https://www.data.gouv.fr/api/1/harvest/sources/?page='+str(page))
    maxpage = int(r.json()['total'] / r.json()['page_size']+1)
    arr = []
    cpt = 0
    page = 0
    for i in range(maxpage):
        r = requests.get('https://www.data.gouv.fr/api/1/harvest/sources/?page='+str(page+1))
        print('https://www.data.gouv.fr/api/1/harvest/sources/?page='+str(page+1))
        for d in r.json()['data']:
            cpt = cpt + 1
            if(d['validation']['state'] == 'pending'):
                mydict = {}
                mydict['admin_url'] = 'https://www.data.gouv.fr/fr/admin/harvester/'+d['id']
                mydict['name'] = d['name']
                mydict['url'] = d['url']
                if d['organization']:
                    mydict['orga'] = 'Organisation ' + d['organization']['name']
                    mydict['orga_url'] = 'https://www.data.gouv.fr/fr/organizations/' + d['organization']['id']
                if d['owner']:
                    mydict['orga'] = 'Utilisateur ' + d['owner']['first_name'] + ' ' + d['owner']['last_name']
                    mydict['orga_url'] = 'https://www.data.gouv.fr/fr/users/' + d['owner']['id']
...
def get_preview_state_from_api(ti):
    ...
    for item in list_pendings:
        try:
            r = requests.get('https://www.data.gouv.fr/api/1/harvest/source/{}/preview'.format(item['id']), timeout=60, headers=headers)
            print(r.json())
...
def publish_mattermost_harvester(ti):
    list_pendings=ti.xcom_pull(key='list_pendings_complete', task_ids='get_preview_state')
    ...
    list_pendings_other = [lp for lp in list_pendings if lp['preview'] not in ['timeout', 'done', 'failed']]


    text = ':mega: Rapport hebdo sur l\'état des moissonneurs en attente : \n - {} moissonneurs en attente \n - {} moissonneurs en attente dont la preview fonctionne \n - {} moissonneurs en attente dont la preview n\'aboutit pas (timeout de 60 secondes) \n - {} moissonneurs en attente dont la preview failed \n - {} moissonneurs dont la preview est dans un autre statut \n \n\nListe des moissonneurs en pending dont la preview fonctionne : \n'.format(len(list_pendings), len(list_pendings_done), len(list_pendings_timeout), len(list_pendings_failed), len(list_pendings_other))

    for lp in list_pendings_done:
        print(lp)
        text = text + ' - [{}]({}) - Moissonneur [{}]({}) - Lien vers [l\'espace Admin]({}) \n'.format(lp['orga'], lp['orga_url'], lp['name'], lp['url'], lp['admin_url'])
    ...
    text = text + 'Avant validation, pensez à consulter [le pad des moissonneurs à laisser en attente de validation]({}) \n'.format(PAD_AWAITING_VALIDATION)
